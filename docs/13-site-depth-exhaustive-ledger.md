...
| :-- | :-- | :-- |
| `current direct enhancement` | Already landed on the current academic decision workspace contract | Canvas inbox messages; richer assignment submission/score context; Gradescope graded submission annotation detail plus graded-copy/history/regrade action hints; EdStem thread summary/category context; EdStem course resources plus landed lesson-summary links; MyUW notice/class-exam detail |
...
| Canvas | `resources` | `/api/v1/courses/:course_id/files` plus `include[]=syllabus_body` on `/api/v1/courses`, with landed-on-main enrichment from `modules / groups / media_objects` | `official` | Implemented on the shared `Resource` contract with file metadata, visible download links when available, syllabus-summary resources, and landed-on-main module/group/media carriers, including module references for `Page / ExternalUrl / File / Assignment / Discussion / Quiz / SubHeader` | Manual live validation still required for current session truth | full module-tree parity, deeper group-scoped carriers, richer recording parity, and instructor-feedback carriers are still deeper-next-wave work | keep file resources and landed module/group/media carriers truthful on the existing `Resource` contract; do not overclaim full module tree parity or durable recording support | `current direct enhancement` |
...
| EdStem | `resources` | session-backed `api/courses/:course_id/resources` with authenticated course-scoped download URL normalization, landed-on-main `/lessons` DOM normalization for summary-first lesson links, plus current local `api/lessons/:lesson_id?view=1` lesson-detail enrichment that promotes lesson slides into grouped `Resource` items on the same shared contract | `internal`, `session-backed`, `dom-fallback` | Implemented with canonical `Resource` entities, file metadata, optional session-backed download URLs, summary-first lesson links, and current local lesson-detail slide enrichment on the shared schema/storage/export chain | Manual live validation now has a reviewed redacted resources API fixture, authenticated raw capture corroboration for the current resource carrier, fresh authenticated DOM proof for `/lessons` rows, and current local proof that lesson pages expose a session-backed `api/lessons/:lesson_id?view=1` carrier with grouped lesson-slide materials | broader grouped-material semantics or richer download UX beyond lesson-detail slide materials on the current `Resource` contract | keep the current API-first resource carrier on the shared `Resource` contract, treat lesson links as summary-first lesson resources, promote lesson slides as grouped lesson materials when the detail carrier is present, and do not overclaim broader grouped-material parity or stable public APIs | `current direct enhancement` |
...
| :-- | :-- | :-- |
| MyPlan | `partial shared landing` | shared planning substrate plus extension/web `Planning Pulse` adoption are real, and the repo now adds a capture path for `plan / audit` pages into that same substrate. Fresh canonical repo-owned browser proof confirms that both `plan` and `audit` still open on the live `Profile 1` lane, but this is still not full shipped site/runtime parity, automatic shared capture parity, or a public API claim |
| DARS | `deepwater runtime landing` | a landed summary lane now exists through the shared planning substrate and administrative summary lane, but it is still not a standalone DARS site/runtime contract |
| Transcript / finaid / accounts / tuition_detail | `deepwater runtime landing` | landed summary lanes now exist on the admin high-sensitivity substrate. `tuition_detail` is now stronger than a page-open placeholder because the current statement carrier can summarize quarter, due/balance posture, classification, credit hours, totals, and fee-breakdown visibility, but these lanes are still export-first and do not yet grant truthful full-detail/runtime parity or AI-readable records |
| Profile | `deepwater runtime landing` | a landed summary lane now exists on the admin high-sensitivity substrate using the current MyUW profile page, but it is still not a standalone personal-record domain and remains export-first / AI-blocked |
...
| Time Schedule | `partial shared landing` | valuable catalog/schedule context is landed on the public course-offerings carrier, and main now supports an active-tab SLN detail fallback for richer section proof. It is still clearly separate from registration automation and full upstream parity |
...
| :-- | :-- | :-- |
| `@campus-copilot/gradescope-api`, `@campus-copilot/edstem-api`, `@campus-copilot/myuw-api` | Real repo-public preview libs over the current snapshot/shared-contract path | They are integration-facing preview packaging, not site-proof by themselves |
