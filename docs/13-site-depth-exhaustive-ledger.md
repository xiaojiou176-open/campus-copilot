...
| EdStem | `resources` | session-backed `api/courses/:course_id/resources` with authenticated course-scoped download URL normalization, landed-on-main `/lessons` DOM normalization for summary-first lesson links, plus current local `api/lessons/:lesson_id?view=1` lesson-detail enrichment on the same `Resource` contract | `internal`, `session-backed`, `dom-fallback` | Implemented with canonical `Resource` entities, file metadata, optional session-backed download URLs, summary-first lesson links, and current local lesson-detail enrichment on the shared schema/storage/export chain | Manual live validation now has a reviewed redacted resources API fixture, authenticated raw capture corroboration for the current resource carrier, fresh authenticated DOM proof for `/lessons` rows, and current local proof that lesson pages expose a session-backed `api/lessons/:lesson_id?view=1` carrier | broader grouped-material semantics or richer download UX beyond the current `Resource` contract | keep the current API-first resource carrier on the shared `Resource` contract, treat lesson links as summary-first lesson resources, and use lesson-detail proof to decide contract promotion; do not overclaim grouped-material parity or stable public APIs | `current direct enhancement` |
