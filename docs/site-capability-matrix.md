...
| EdStem | `resources` | session-backed `https://us.edstem.org/api/courses/:course_id/resources` with API-first normalization of file metadata and optional course-scoped download URLs, plus current-worktree `/us/courses/:course_id/lessons` DOM normalization for summary-first lesson links | `internal`, `session-backed`, `dom-fallback` | Implemented on the shared `Resource` contract | Manual live validation now has a reviewed redacted resources API fixture, authenticated raw capture corroboration for the current resource carrier, and fresh authenticated DOM proof that `/lessons` rows can normalize into summary-first lesson resources | broader grouped-material semantics or richer download UX remain selective if a later contract needs them |
...
| Profile | current-user profile visibility | read-only only; summary-first and export-first | summary-first runtime lane now exists on the admin high-sensitivity substrate using the current MyUW profile page carrier. Still not a standalone personal-record domain or AI-readable detail lane |
...
| Tuition detail | tuition statement summary | read-only only; summary-first and export-first | summary-first runtime lane now exists on the admin high-sensitivity substrate using the current tuition statement page carrier. Current truth is now stronger than a page-open placeholder: the lane can summarize quarter, due/balance posture, tuition classification, credit hours, charges/payments totals, aid-to-account, and whether a mandatory-fee breakdown table is visible. Still not a standalone billing-detail or AI-readable document lane |
| Time Schedule | catalog/schedule lookup | read-only only; must not turn into registration automation | shared runtime landing now exists on the public course-offerings carrier with shared site labels/runtime fan-out, and the current working tree now also supports an active-tab SLN detail fallback on `sdb.admin.washington.edu/timeschd/uwnetid/sln.asp?...`. Still limited read-only expansion support, not registration support or full upstream-site parity |
...
| EdStem | grouped resource bundles or richer download UX beyond the current `Resource` contract | selective gap | The current API-backed `Resource` path is shipped, and current worktree now also lands summary-first lesson links from `/lessons`, but broader grouped-material semantics or richer download affordances are still not formal current scope. |
...
| EdStem resources | current shipped matrix | current formal scope | authenticated `api/courses/:course_id/resources` responses now normalize into canonical `Resource` entities with file metadata and optional session-backed download URLs, and current worktree also lands summary-first lesson links from `/lessons` |
...
| EdStem | grouped resource bundles / richer download UX beyond the current `Resource` contract | `selective gap` | current API-backed resources are shipped, and current worktree now also lands summary-first lesson links from `/lessons`, but broader grouped-material semantics are still not formal current truth |
