...
| EdStem | grouped resource bundles or richer download UX beyond the current `Resource` contract | selective gap | The current API-backed `Resource` path is shipped, and landed-on-main lesson links from `/lessons` now also feed the same summary-first contract, but broader grouped-material semantics or richer download affordances are still not formal current scope. |
...
| EdStem | grouped resource bundles / richer download UX beyond the current `Resource` contract | `selective gap` | current API-backed resources are shipped, and landed-on-main lesson links from `/lessons` now also feed the same summary-first contract, but broader grouped-material semantics are still not formal current truth |
