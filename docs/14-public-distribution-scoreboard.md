...
| `@campus-copilot/mcp-server` | `public-ready (repo-local)`, `registry submitted`, `public discovery hit for v0.1.0` | `pnpm --filter @campus-copilot/mcp-server build && pnpm --filter @campus-copilot/mcp-server test` | current `main` now exceeds the published v0.1.0 `.mcpb` artifact; refresh the release-hosted bundle and republish metadata before treating the listing as current-head truth; packet lives in `packages/mcp-server/registry-submission.packet.json` |
...
| :-- | :-- | :-- | :-- | :-- |
| MCP Registry | Yes | [registry.modelcontextprotocol.io](https://registry.modelcontextprotocol.io/) | the official API search now returns `io.github.xiaojiou176-open/campus-copilot-mcp` as `active`, but that listing still reflects the older v0.1.0 artifact | refresh the `.mcpb` asset and republish server metadata so the public listing matches current `main` |
