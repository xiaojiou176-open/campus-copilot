...
function buildMyUwSiteView(snapshot: SnapshotLike, limit = 50) {
  const bySite = (entry: SnapshotSiteEntity) => entry?.site === 'myuw';
...
export function buildMyUwSnapshotView(snapshot: SnapshotLike, limit = 50) {
  return buildMyUwSiteView(snapshot, limit);
}
