...
function buildEdStemSiteView(snapshot: SnapshotLike, limit = 50) {
  const bySite = (entry: SnapshotSiteEntity) => entry?.site === 'edstem';
...
export function buildEdStemSnapshotView(snapshot: SnapshotLike, limit = 50) {
  return buildEdStemSiteView(snapshot, limit);
}
