...
export function useEntityCounts(db: CampusCopilotDB = openCampusDb, refreshKey?: number) {
  return useLiveQuery(() => getEntityCounts(db), [db, refreshKey]);
}

export function useSyncState(db: CampusCopilotDB = openCampusDb, refreshKey?: number) {
  return useLiveQuery(() => getLatestSyncState(db), [db, refreshKey]);
}

export function useSiteSyncStates(db: CampusCopilotDB = openCampusDb, refreshKey?: number) {
  return useLiveQuery(() => getSiteSyncStates(db), [db, refreshKey]);
}

export function useLatestSyncRuns(limit = 8, db: CampusCopilotDB = openCampusDb, refreshKey?: number) {
...
export function useAllAssignments(db: CampusCopilotDB = openCampusDb, refreshKey?: number) {
  return useLiveQuery(() => getAllAssignments(db), [db, refreshKey]);
}

export function useAllCourses(db: CampusCopilotDB = openCampusDb, refreshKey?: number) {
  return useLiveQuery(() => getAllCourses(db), [db, refreshKey]);
}

export function useAllResources(db: CampusCopilotDB = openCampusDb, refreshKey?: number) {
  return useLiveQuery(() => getAllResources(db), [db, refreshKey]);
}

export function useAllAnnouncements(db: CampusCopilotDB = openCampusDb, refreshKey?: number) {
  return useLiveQuery(() => getAllAnnouncements(db), [db, refreshKey]);
}

export function useAllMessages(db: CampusCopilotDB = openCampusDb, refreshKey?: number) {
  return useLiveQuery(() => getAllMessages(db), [db, refreshKey]);
}

export function useAllGrades(db: CampusCopilotDB = openCampusDb, refreshKey?: number) {
  return useLiveQuery(() => getAllGrades(db), [db, refreshKey]);
}

export function useAllEvents(db: CampusCopilotDB = openCampusDb, refreshKey?: number) {
  return useLiveQuery(() => getAllEvents(db), [db, refreshKey]);
}

export function useSiteEntityCounts(site: Site, db: CampusCopilotDB = openCampusDb, refreshKey?: number) {
  return useLiveQuery(() => getSiteEntityCounts(site, db), [site, db, refreshKey]);
}

export function useAllSiteEntityCounts(db: CampusCopilotDB = openCampusDb, refreshKey?: number) {
  return useLiveQuery(() => getAllSiteEntityCounts(db), [db, refreshKey]);
}

export function useSiteSyncState(site: Site, db: CampusCopilotDB = openCampusDb, refreshKey?: number) {
  return useLiveQuery(() => getSyncStateBySite(site, db), [site, db, refreshKey]);
}

export function useTodaySnapshot(now: string, db: CampusCopilotDB = openCampusDb, refreshKey?: number) {
  return useLiveQuery(() => getTodaySnapshot(now, db), [now, db, refreshKey]);
}

export function usePriorityAlerts(now: string, db: CampusCopilotDB = openCampusDb, refreshKey?: number) {
...
export function useRecentChangeEvents(limit = 20, db: CampusCopilotDB = openCampusDb, refreshKey?: number) {
  return useLiveQuery(() => getRecentChangeEvents(limit, db), [limit, db, refreshKey]);
}

export function useFocusQueue(now: string, db: CampusCopilotDB = openCampusDb, refreshKey?: number) {
  return useLiveQuery(() => getFocusQueue(now, db), [now, db, refreshKey]);
}

export function useWeeklyLoad(now: string, db: CampusCopilotDB = openCampusDb, refreshKey?: number) {
...
export function useAllPlanningSubstrates(db: CampusCopilotDB = openCampusDb, refreshKey?: number) {
