...
RESPONSE_FILE="$(mktemp "$TEMP_DIR/provider-roundtrip-response.XXXXXX")"
LOG_FILE="$(mktemp "$TEMP_DIR/provider-roundtrip-log.XXXXXX")"
SERVER_PID=""
...
for _attempt in 1 2 3; do
  ...
  stop_server
  printf '{\n  "status": "failed",\n  "reason": "api_server_never_became_healthy",\n  "attempt": %s,\n  "port": %s\n}\n' "$_attempt" "$PORT" >&2
  exit 1
done

if [[ "$SERVER_READY" != "1" ]]; then
  stop_server
  printf '{\n  "status": "failed",\n  "reason": "api_server_never_became_healthy_after_retry_budget",\n  "attempts": 3,\n  "port": %s\n}\n' "$PORT" >&2
  exit 1
fi
