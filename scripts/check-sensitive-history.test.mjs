...
test('reuses fixture-sensitive rules when reachable history contains live fixture residue', () => {
  ...
  const failures = collectSensitiveHistoryFailures({
    historyEntries: [{ objectId: 'cafebabefeed', file: 'packages/adapters-gradescope/src/__fixtures__/live/example.json' }],
    readObject: () => Buffer.from(fixture, 'utf8'),
  });

  assert.match(
    failures.join('\n'),
    /history_fixture_stable_identifier_key:packages\/adapters-gradescope\/src\/__fixtures__\/live\/example\.json@cafebabefeed:2/,
  );
  assert.match(
    failures.join('\n'),
    /history_fixture_real_host_url:packages\/adapters-gradescope\/src\/__fixtures__\/live\/example\.json@cafebabefeed:3/,
  );
});
