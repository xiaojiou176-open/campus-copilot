...
describe('ask ai panel guardrails', () => {
  it('renders manual-only red-zone copy and a default-disabled advanced material switch', () => {
    ...
    expect(html).toContain('Check the evidence first');
    ...
    expect(html.indexOf('Check the evidence first')).toBeLessThan(html.indexOf('Question box'));
    expect(html.indexOf('Check the evidence first')).toBeLessThan(html.indexOf('What AI can see'));
    expect(html.indexOf('What AI can see')).toBeLessThan(html.indexOf('Academic safety guardrails'));
    expect(html.indexOf('Check the evidence first')).toBeLessThan(html.indexOf('Advanced material analysis'));
  });
});
