...
- `Switchyard` 是后续可接入的本地 provider runtime 方向；在它完工并明确接入前，不要把 `OpenAI Web` / `Gemini Web` 网页登录当成当前 CampusCopilot 主线 blocker。
...
- `CampusCopilot` 的正式身份始终是：**local-first academic decision workspace**。
